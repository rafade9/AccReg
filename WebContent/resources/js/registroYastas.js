...
$(document).ready(function(){
	...
	mensajesYastas[99] = "<msg class='blackMsgClass'>Su operaci&oacute;n no se pudo completar. Intente nuevamente.</msg>";
...
//	              	$.getJSON("/FormularioProspectacion/getPostalCode.htm?cp=" + parseInt(this.value, 10), function(data) {//produccion
						$.getJSON("/FormularioProspectacion/resources/codPostal.json", function(data) {//desarrollo	
							...
							if (data.status === "Ok") {
								...
								$("#delegacion").val("");
								$("#ciudad").val("");
								$("#colonia").replaceWith('<input id="colonia" name="colonia" class="inputText requerido alfanumerico alfanumericoBloquear texGris09_13 mayusculas" type="text" maxlength="30"/>');
...
//								});		
		...
		$("#colonia").change(function(){
			if($('#colonia').val() == "###"){
				$("#colonia").replaceWith('<input id="colonia" name="colonia" class="inputText requerido  texGris09_13 mayusculas" type="text" maxlength="30"/>');
				$("#colonia option").remove();
				$("#colonia").val("");
//				$('#colonia').keyup(function(){	
//					$(this).val($(this).val().toUpperCase());
